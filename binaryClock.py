...
def signal_term_handler(signal, frame):
    if signal == 2:
        print("\nCtrl + C DEADDDDDDDDDDDDDDDDDD")
    if signal == 15:
        print("\nFUCCCCCCCCCCCCCCCCCK Got killed")
...
# Set initial rotation based on command-line argument
def set_initial_rotation(degrees):
    if degrees in [0, 90, 180, 270]:
        hat.set_rotation(degrees)
        print(f"Display rotated to {degrees} degrees")
    else:
        print("Invalid rotation angle. Only 0, 90, 180, and 270 degrees are supported.")

# Parse the -r argument for rotation
parser = argparse.ArgumentParser(description='Rotate the display on start.')
parser.add_argument('-r', '--rotate', type=int, help='Set display rotation (0, 90, 180, 270 degrees)', default=0)
args = parser.parse_args()

# Set rotation based on the provided argument
set_initial_rotation(args.rotate)
        ...
        if event.direction == 'up':
            ...
            if special_display_active:
                ...
                if rotation == 0:
                    ...
                    print("Cannot rotate with the given input")
